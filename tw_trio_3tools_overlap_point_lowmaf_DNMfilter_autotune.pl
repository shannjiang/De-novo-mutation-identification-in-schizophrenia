...
$j = 0;
for (0..2000) {
open $in_fh,'<',"/data1/sjiang4/denovo/jointcall_trio/3tools_overlap_lowmaf_DNM_mr1e8/$sampleID_matrix[$i][0]_$sampleID_matrix[$i][1]_point_lowmaf.vcf";
open $out_fh,'>',"/data1/sjiang4/denovo/jointcall_trio/3tools_overlap_lowmaf_DNMfd_mr1e8_autotune/$sampleID_matrix[$i][0]_$sampleID_matrix[$i][1]_point_lowmaf.mr1e8.fd.vcf";
...
system("cat /data1/sjiang4/denovo/jointcall_trio/3tools_overlap_lowmaf_DNMfd_mr1e8_autotune/$sampleID_matrix[$i][0]_$sampleID_matrix[$i][1]_point_lowmaf.mr1e8.fd.vcf | wc -l > /data1/sjiang4/denovo/jointcall_trio/3tools_overlap_lowmaf_DNMfd_mr1e8_autotune/$sampleID_matrix[$i][0]_$sampleID_matrix[$i][1]_point_lowmaf.linecount");
open $file_fh,'<',"/data1/sjiang4/denovo/jointcall_trio/3tools_overlap_lowmaf_DNMfd_mr1e8_autotune/$sampleID_matrix[$i][0]_$sampleID_matrix[$i][1]_point_lowmaf.linecount";
@file_array = <$file_fh>;
chomp($file_array[0]);
#print "$file_array[0] ";
